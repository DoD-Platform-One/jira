...
![Version: 0.1.0-bb.10](https://img.shields.io/badge/Version-0.1.0--bb.10-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 8.18.1](https://img.shields.io/badge/AppVersion-8.18.1-informational?style=flat-square)
...
| image.repository | string | `"registry1.dso.mil/ironbank/atlassian/jira-data-center/jira-node"` |  |
| image.imagePullSecrets | list | `[{"name":"private-registry"}]` | Optional image repository pull secret |
| image.pullPolicy | string | `"IfNotPresent"` |  |
| image.tag | string | `"8.18.1"` | The docker image tag to be used. Defaults to the Chart appVersion. |
...
| database.driver | string | `"org.postgresql.Driver"` | The Java class name of the JDBC driver to be used, e.g. org.postgresql.Driver If not specified, then it will need to be provided via browser during initial startup. |
...
| database.credentials.usernameSecretKey | string | `nil` | The key in the Secret used to store the database login username |
| database.credentials.passwordSecretKey | string | `nil` | The key in the Secret used to store the database login password |
...
| volumes.sharedHome.efs | bool | `false` | If AWS efs is utilized, please ensure efs to set 'efs' to 'true' and provide the aws efs-id volume to create pv |
