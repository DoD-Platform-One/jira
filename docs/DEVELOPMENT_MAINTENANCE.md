...
1.  Visit the upstream Helm chart docs to see if there’s a new release ([link](https://atlassian.github.io/data-center-helm-charts/)). Confirm the latest Jira chart on the GitHub Releases page ([Latest Releases](https://github.com/atlassian/data-center-helm-charts/releases)). If a new version is available, proceed to Step 3.
2. Bump the Jira dependency version in chart.yaml, then run ```helm dependency update ./chart``` to pull in the new chart.
3. Update version references for the Chart. `version` should be `<version>-bb.0` (ex: `1.14.3-bb.0`) and `appVersion` should be `<version>` (ex: `1.14.3`). Also validate that the BB annotation for Jira is updated.
4. Review the [upstream release notes](https://github.com/atlassian/data-center-helm-charts) for the update you are going to, as well as any versions skipped over between the last BB release and this one. Note any breaking changes and new features.
